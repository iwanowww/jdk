...
ATTRIBUTE_ALIGNED(16) uint64_t _KEY_SHUFFLE_MASK[] = { 0x0405060700010203UL, 0x0c0d0e0f08090a0bUL };
address StubGenerator::KEY_SHUFFLE_MASK = (address)_KEY_SHUFFLE_MASK;

ATTRIBUTE_ALIGNED(16) uint64_t _GHASH_BYTE_SWAP_MASK[] = { 0x08090a0b0c0d0e0fUL, 0x0001020304050607UL };
address StubGenerator::GHASH_BYTE_SWAP_MASK = (address)_KEY_SHUFFLE_MASK;

ATTRIBUTE_ALIGNED(16) uint64_t _GHASH_LONG_SWAP_MASK[] = { 0x0f0e0d0c0b0a0908UL, 0x0706050403020100UL };
address StubGenerator::GHASH_LONG_SWAP_MASK = (address)_GHASH_LONG_SWAP_MASK;
...
#ifndef PRODUCT
void StubGenerator::inc_counter_np_(int& counter) {
...
static int& get_profile_ctr(int shift) {
  switch (shift) {
    ...
    default: return SharedRuntime::_jlong_array_copy_ctr;
  }
}
...
address StubGenerator::generate_disjoint_copy_avx3_masked(address* entry, const char *name, int shift,
...
//   c_rarg2   - element count, treated as ssize_t, can be zero
//
address StubGenerator::generate_conjoint_long_oop_copy(bool aligned, bool is_oop,
...
void StubGenerator::generate_type_check(Register sub_klass,
  ...
  Label L_failure;
  __ check_klass_subtype_fast_path(sub_klass, super_klass, noreg,        &L_success, &L_failure, NULL, super_check_offset);
...
//
address StubGenerator::generate_checkcast_copy(const char *name, address *entry,
                                               bool dest_uninitialized) {
...
address StubGenerator::generate_cipherBlockChaining_encryptAESCrypt() {
  ...
  load_key(xmm_temp, key, 0xe0, rscratch1); 
...
address StubGenerator::generate_electronicCodeBook_encryptAESCrypt() {
  ...
  aesecb_encrypt(from, to, key, len, rscratch1);
...
address StubGenerator::generate_avx_ghash_processBlocks() {
  ...
  __ enter();
  // Save state before entering routine
  avx_ghash(state, htbl, data, blocks, rscratch1);
  __ leave(); // required for proper stackwalking of RuntimeStub frame
  __ ret(0);
  return start;
}
