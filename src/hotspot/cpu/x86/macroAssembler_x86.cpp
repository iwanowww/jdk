...
//  lea(table, Address(table, Array<Klass*>::base_offset_in_bytes()));
  ...
  movptr(rtmp3, Address(table, rtmp3, Address::times_ptr, Array<Klass*>::base_offset_in_bytes() + BytesPerWord /*+1*/)); // probe2; kills idx2
  cmpptr(super_klass, rtmp3);  // if (probe2 == k)  success!
  jcc(Assembler::equal, *L_success);

  testptr(rtmp3, rtmp3); // if (probe2 == NULL) failure
  jcc(Assembler::zero, *L_failure);
...
//  const Register table = rtmp2;
  const Register count = rtmp3;

  movptr(table, Address(sub_klass, in_bytes(Klass::secondary_supers_table_offset())));
  movptr(count, Address(table, Array<Klass*>::length_offset_in_bytes())); // total count

  lea(table, Address(table, mask, Address::times_ptr, Array<Klass*>::base_offset_in_bytes())); // start of linear section
  subl(count, mask);
  jcc(Assembler::zero, *L_failure);
