...
inline constexpr Register as_Register(int encoding) {
  if (0 <= encoding && encoding < RegisterImpl::number_of_declared_registers) {
...
inline constexpr const FloatRegisterImpl* FloatRegisterImpl::first() { return all_FloatRegisterImpls + 1; }
...
extern PRegisterImpl all_PRegisterImpls[PRegisterImpl::number_of_registers + 1] INTERNAL_VISIBILITY;
inline constexpr const PRegisterImpl* PRegisterImpl::first() { return all_PRegisterImpls + 1; }

class PRegister {
  friend class PRegisterImpl;

  const PRegisterImpl* _ptr;

  constexpr PRegister(const PRegisterImpl* ptr, bool unused) : _ptr(ptr) {}

public:
  inline friend constexpr PRegister as_PRegister(int encoding);

  constexpr PRegister() : _ptr(as_PRegisterImpl(-1)) {} // pnoreg
...
inline constexpr PRegister as_PRegister(int encoding) {
  if (0 <= encoding && encoding < PRegisterImpl::number_of_registers) {
    return PRegister(as_PRegisterImpl(encoding), false);
  }
  return pnoreg;
}

inline PRegister PRegisterImpl::successor() const {
