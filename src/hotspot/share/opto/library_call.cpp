...
bool LibraryCallKit::try_to_inline(int predicate) {
  ...
  case vmIntrinsics::_getFlatValue:             return inline_unsafe_flat_access(id, vmIntrinsics::UNSAFE_MO_PLAIN);
  case vmIntrinsics::_putFlatValue:             return inline_unsafe_flat_access(id, vmIntrinsics::UNSAFE_MO_PLAIN);
...
bool LibraryCallKit::inline_unsafe_access(vmIntrinsics::ID id,
  ...
  if (!is_store) {
    ...
    if (type == T_BOOLEAN) {
      ...
      p = _gvn.transform(new MoveF2INode(p));
    } else if (type == T_DOUBLE) {
      p = _gvn.transform(new MoveD2LNode(p));
    }
    if (type_is_narrow_int) {
...
bool LibraryCallKit::inline_unsafe_flat_access(vmIntrinsics::ID id, vmIntrinsics::MemoryOrder mo) {
  const bool is_store = (id == vmIntrinsics::_putFlatValue);
#ifdef ASSERT
  {
    ResourceMark rm;
    // Check the signatures.
  ...
  assert(mo == vmIntrinsics::UNSAFE_MO_PLAIN, "Only plain accesses for now");
  if (callee()->is_static()) {
...
bool LibraryCallKit::inline_native_try_update_epoch() {
  ...
  decorators |= mo_decorator_for_access_kind(vmIntrinsics::UNSAFE_MO_VOLATILE);
