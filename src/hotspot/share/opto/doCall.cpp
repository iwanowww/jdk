      ...
      if (singleton != NULL &&
        ...
        if (cha_monomorphic_target != NULL &&
          ...
          if (hit_cg != NULL && cg != NULL) {
            dependencies()->assert_unique_concrete_method(declared_interface, cha_monomorphic_target, caller->holder(), declared_interface);
            return cg;
          }
        }
      }
...
ciMethod* Compile::optimize_inlining(ciMethod* caller, ciInstanceKlass* klass, ciKlass* holder,
  ...
  if (cha_monomorphic_target != NULL) {
    ...
    if (!cha_monomorphic_target->can_be_statically_bound(actual_receiver)) {
      ...
      dependencies()->assert_unique_concrete_method(actual_receiver, cha_monomorphic_target, calling_klass, holder);
    }
